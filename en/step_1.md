...
Most of the information in this guide applies to both Life in Space and Life on Earth experiments, however  there are a few differences that you need consider depending on which experiment theme you are entering.
...
+ Your program should display a helpful message or image on the SenseHAT's LED matrix so that nearby astronauts know that an experiment is running. This should change regularly to indicate that everything is running correctly.
