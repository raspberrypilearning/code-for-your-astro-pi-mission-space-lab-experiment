...
To easily create and write to a CSV file, we recommend using `csv` which is included in the Python standard library. Here is a simple example which involves constructing the name of the data file, opening it with write permissions and adding a single header row before iteratively writing an additional row of sensor data every 60 seconds.
